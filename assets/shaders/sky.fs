...
void main()
{
    ...
    float cloudAltitude =
        smoothstep(0.045, 0.13, viewDirection.y)*
        (1.0 - smoothstep(0.32, 0.52, viewDirection.y));
    vec2 cloudPosition = vec2(
        atan(viewDirection.z, viewDirection.x)*1.8 +
            timeSeconds*0.006,
        viewDirection.y*9.0);
    ...
    float clouds =
        ...
        cloudAltitude;
    vec3 cloudColor = mix(horizonColor, vec3(0.94, 0.96, 0.95), 0.72);
